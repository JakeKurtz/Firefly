...
{
    float a2 = r * r * r * r;
    float NH2 = pow(max(0.0, dot(n, h)), 2);
...
vec3 f(vec3 wi, vec3 wo, vec3 n, vec3 a, vec3 f0, float r, float m)
{
    ...
    float n_dot_wi = max(dot(n, wi), 0.f);
    float n_dot_wo = max(dot(n, wo), 0.f);

    float D = ggxtr_ndf(n, wh, r);
    float G = geo_atten(wi, wo, n, r);
    vec3 F = fresnel(f0, wh, wi);
...
void main()
{
    ...
    //vec4 FragPosLightSpace = lightSpaceMatrix[0] * vec4(FragPos, 1.f);
    //FragColor = vec4(max(vec3(shadow(FragPosLightSpace, 0)), 0.5)*ambient * cam_exposure, 1.0);
}
