...
public:
	...
	__device__ virtual bool hit(const dRay& ray, float& tmin, Isect& isect) const
	{
	...
	__device__ virtual bool hit(const dRay& ray) const
	{
		float t = dot((point - ray.o), normal) / dot(ray.d, normal);
	...
	__device__ virtual bool shadow_hit(const dRay& ray, float& tmin) const
	{
		if (!shadows)
			return (false);
