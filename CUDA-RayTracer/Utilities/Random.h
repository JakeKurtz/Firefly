...
__device__ unsigned int random_count = 0;
...
__device__ float random()
{
	// Old slow ass random
	/*
	uint32_t threadId = (blockIdx.x + blockIdx.y * gridDim.x) * (blockDim.x * blockDim.y) + (threadIdx.y * blockDim.x) + threadIdx.x;
	...
	*/
	...
	random_count += 10;
	unsigned int seed = ((i * 1024 + j) * 100) + random_count;
