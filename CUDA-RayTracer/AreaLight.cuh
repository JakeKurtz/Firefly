...
public:
	...
	__device__ virtual float3 L(Isect& isect, float3 wi, float3 sample_point)
	{
		float3 light_normal = object_ptr->get_normal(sample_point);
		float n_dot_d = dot(-light_normal, wi);

		if (n_dot_d > 0.0)
			return (material_ptr->get_Le(isect));
		else
			return (make_float3(0, 0, 0));
	};
	...
	__device__ virtual bool in_shadow(const Ray& ray, const ShadeRec& sr) const
	{
		...
		return shadow_hit(ray, ts, sr.bvh);
	};

	__device__ virtual bool in_shadow(const Ray& ray, LinearBVHNode* __restrict nodes) const
	{
		float3 sample_point = object_ptr->sample();
		...
		return shadow_hit(ray, ts, nodes);
	};
